...
module.exports = app => {
  ...
  if (typeof app.config.sequelize === 'function') {
    new Promise(function(resolve) {
      let config = app.config.sequelize();
      resolve(config);
    }).then(async data => {
      if (!(data instanceof Array))
        data = [ data ];
      installConfig(app, data, true);
    });
  } else {
    if (!(app.config.sequelize instanceof Array))
      cfg = [ cfg ];
      installConfig(app, cfg);
  }

  Object.defineProperty(app.Sequelize, 'authenticated', {
...
function installConfig(app, cfg) {
  ...
  if (arguments.length <= 2)
    ...
    for (let i = 0; i < cfg.length; i++) {
      ...
      if (cfg && typeof cfg.authenticated === "function") cfg.authenticated(app.models[i]);
    }
}
